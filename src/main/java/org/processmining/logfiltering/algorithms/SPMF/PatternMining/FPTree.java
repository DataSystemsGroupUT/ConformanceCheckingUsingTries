...
public class FPTree {
	 ...
	 * @param i 
	 */
	public void addTransaction(List<Integer> transaction, int i) {
	...
	void addPrefixPath(List<FPNode> prefixPath, Map<Integer, Integer> mapSupportBeta, int relativeMinsupp) {
		...
		List<Integer> pathTraceList= prefixPath.get(0).traces; 
		...
		for(int i = prefixPath.size() -1; i >=1; i--){ 
			...
			if(mapSupportBeta.get(pathItem.itemID) >= relativeMinsupp){
				...
				if(child == null){ 
					...
					newNode.traces= pathTraceList; //set its traces
					currentNode.childs.add(newNode);
					currentNode = newNode;
					// We update the header table.
					// and the node links
					...
					child.traces.addAll(pathTraceList);
					currentNode = child;
				}
			}
		}
