...
import org.processmining.logfiltering.algorithms.ICC.*;
    ...
    public static String apply(XLog inputLog, Petrinet net, MatrixFilterParameter parameters, TransEvClassMapping mapping2) {
        ...
        for (int i = 0; i < VariantFreq.length; i++) {
            ...
            for (int j = 0; j < DFRelations.length - 1; j++) {
                DF = Character.toString(DFRelations[j]) + DFRelations[j + 1];
        ...
        switch (SelectiuonType) {
            ...
            case KCenterApprox:
                ...
                for (int i = 0; i < (1 - Threshold) * VariantInd.length; i++) {
                    ...
                    SampledList.remove((Integer) VariantInd[TempIndex]);
            ...
            case FastAlignment: {
                ...
                Replayer replayer = new Replayer(RepParameters2, net, initialMarking, finalMarking, classes, mapping, false);
...
////// Except for Binary alignment method
        ...
        Replayer replayer = new Replayer(RepParameters, net, initialMarking, finalMarking, classes, mapping, false);
...
//					}
        ...
        for (Iterator t = T.iterator(); t.hasNext(); ) {
            ...
            AsyncrousDistribution.put(string, AsyncrousMoves.get(string) * 1.0 / (SyncrousMoves.get(string) + AsyncrousMoves.get(string)));
        }
        ...
        String outp1 = (approximation / LogSize) + ">>" + (upperBound / LogSize) + ">>" + (lowerBound / LogSize) + ">>" +
                ...
                ">>" + ModelBehaviorSim.size() + ">>" + PreprocessTime + ">>" + MBTime + ">>" + (actualFitnessValue - approximation) / LogSize + ">>" + FitnessError + ">>" + avgAlignmentCostError + ">>" + maxAlignmentCostError;
        ...
        String outp2 = "<html><table>\n" +
                "<tbody>\n" +
                "<tr>\n" +
                "<td style=\"width: 200px; font-size: 1.2em;\">Approximation setting :</td>\n" +
                "<td style=\"font-size: 1.2em;\">" + parameters.getPrototypeType() + parameters.getSubsequenceLength() + "</td>\n" +
                "</tr>\n" +
...
/*		double[] AlignmentCosts2= new double [InputLog.size()];
 ...
 for (int i = 0; i < VariantFreq.length; i++) {
    ...
    static class Variant {
        ...
        private final double DoubleScore;
        ...
        for (int j = 1; j < len1; j++) {
            ...
            for (int i = 1; i < len0; i++) {
                ...
                if (match == 0)
                    ...
                    if (Math.min(cost_insert, cost_delete) < cost_replace) {
