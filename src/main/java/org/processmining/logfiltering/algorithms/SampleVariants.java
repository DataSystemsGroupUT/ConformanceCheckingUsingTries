...
import org.processmining.logfiltering.parameters.*;
...
*/
        ...
        if (samplingType != SamplingType.RandomSamplingWithoutLooking) {
            ...
            int Threshold = (int) (VariantSize * (80 * 1.0) / 100);
...
/*			if (samplingType==SamplingType.Random) {
            ...
            switch (returnType) {
                ...
                case InstanceSelection:
                    ...
                    for (int i = 0; i <= VariantSize * ((parameters.getSubsequenceLength() * 1.0) / 100); i++) {
                        sum = sum + 1;
                ...
                case Traces:
                    for (int i = 0; i < chCount; i++) {
                        if (sum <= LogSize * ((parameters.getSubsequenceLength() * 1.0) / 100) && checker == 0) {
                            sum = sum + ActionMaper.get(ReverseMapper.get(Variants[i].getIndex()));
                            FilterHashMaper.put(ReverseMapper.get(Variants[i].getIndex()), i);
                        } else if (sum > LogSize * ((parameters.getSubsequenceLength() * 1.0) / 100)) {
                            checker++;
                        }
                    }
        ...
        switch (returnType) {
            ...
            case Traces:
                ...
                for (XTrace trace : InputLog) { // for each trace
                    ...
                    if (FilterHashMaper.get(tr) != null && sum <= LogSize * ((parameters.getSubsequenceLength() * 1.0) / 100)) {
                        outputLog2.add(trace);
                        sum++;
                    }
                }
