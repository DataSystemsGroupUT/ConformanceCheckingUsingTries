...
import org.processmining.logfiltering.algorithms.ConformanceBoundApproximationAlgoforTest;
...
@Plugin(name = "Conformance Bound Approximation for Test", parameterLabels = {"Event Log", "Petri net", "Parameter Object"}, returnLabels = {
        "Conf Approximation"}, returnTypes = {String.class})
public class ConformamceBoundApproximatorForTestPlugin {
    ...
    public class MatrixFilterWizardPanel extends JPanel {
        ...
        private final SamplingReturnType[] ReturnOptions = SamplingReturnType.values();
        private final PrototypeType[] PrototypeTypes = PrototypeType.values();
        public XEventClassifier[] EventAttribute;
        private final FilterLevel[] chooseName = FilterLevel.values();
        ...
        private final NiceIntegerSlider SubsequenceLength = SlickerFactory.instance().createNiceIntegerSlider("Please Select the Percentages of Variants", 0, 100, 10, Orientation.HORIZONTAL);
        ...
        public MatrixFilterParameter getParameters() {
            return new MatrixFilterParameter(SubsequenceLength.getValue(),
                    (XEventClassifier) comboAtt.getSelectedItem(), SimilarityMeasure.Levenstein, (SamplingReturnType) comboReturnMethod.getSelectedItem(), (PrototypeType) comboProtoTypeSelectionType.getSelectedItem());
        }
...
//
    ...
    public static String run(PluginContext context, XLog log, Petrinet net, TransEvClassMapping mapping, MatrixFilterParameter parameters) {
        ...
        if (parameters.getPrototypeType() != PrototypeType.Sampling) {
            return ConformanceBoundApproximationAlgoforTest.apply(log, net, parameters, mapping2);
        } else {
            return ConformanceBoundApproximationAlgoforTest.apply2(context, log, net, parameters, mapping2);
        }
