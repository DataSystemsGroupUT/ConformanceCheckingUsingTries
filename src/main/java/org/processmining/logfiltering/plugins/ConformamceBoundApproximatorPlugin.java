...
import org.processmining.logfiltering.algorithms.ConformanceBoundApproximationAlgo;
...
@Plugin(name = "Conformance Bound Approximation", parameterLabels = {"Event Log", "Petri net", "Parameter Object"}, returnLabels = {
        "Conf Approximation"}, returnTypes = {String.class})
public class ConformamceBoundApproximatorPlugin {
    ...
    public class MatrixFilterWizardPanel extends JPanel {
        ...
        private final NiceIntegerSlider SubsequenceLength = SlickerFactory.instance().createNiceIntegerSlider("Please Select the Percentages of Variants", 0, 100, 20, Orientation.HORIZONTAL);
...
//
    ...
    public static String run(PluginContext context, XLog log, Petrinet net, TransEvClassMapping mapping, MatrixFilterParameter parameters) {
        ...
        if (parameters.getPrototypeType() != PrototypeType.Sampling) {
            return ConformanceBoundApproximationAlgo.apply(log, net, parameters, mapping2);
        } else {
            return ConformanceBoundApproximationAlgo.apply2(context, log, net, parameters, mapping2);
        }
